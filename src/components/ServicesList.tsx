...
export default function ServicesList({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-3 sm:gap-x-4 md:gap-x-3 gap-y-5 sm:gap-y-6 md:gap-y-5 ${className}`}>
      {items.map((service, i) => {
        ...
        return (
          <AnimateOnScroll key={service.id} delay={i * 50} once={false} className="h-full">
            <div
              className="group relative bg-white rounded-2xl border-t-4 shadow-sm hover:-translate-y-1.5 hover:shadow-[0_10px_28px_rgba(0,0,0,0.09)] transition-[transform,box-shadow] [transition-duration:250ms] ease-out p-3 sm:p-4 md:p-3 h-full flex flex-col gap-2 overflow-hidden"
